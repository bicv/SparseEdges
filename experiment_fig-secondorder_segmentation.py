...
try:
    ...
    fig, a = mp.show_edges(edges=0., image=image, v_min=v_min, v_max=v_max, color='toto', show_phase=False) #
    if not(figpath==None): 
        for ext in FORMATS: 
...
if False:
    ...
    mp = SparseEdges(LogGabor(Image(init_pe())))
    mp.pe.eta_SO = eta_SO
    for mp.pe.dip_scale in np.logspace(-1., 1., N_explore, base=base)*pe.dip_scale:
